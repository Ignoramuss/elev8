...
public class NetworkPolicySpec {
     ...
     * you must specify a policyTypes value that includes "Egress" (since an empty egress list would be ignored).
     */
    @Singular("policyType")
     ...
     * If this field is empty then this NetworkPolicy does not allow any traffic (and serves
     ...
     * across all of the NetworkPolicy objects whose podSelector matches the pod.
     * If this field is empty then this NetworkPolicy limits all outgoing traffic (and serves
     * solely to ensure that the pods it selects are isolated by default).
